...
function filterProductsByPriceRange(products, min, max) {
  if (!Array.isArray(products) || products.length === 0) {
    throw new Error("The products array is empty or not an array");
  }
  if (typeof min !== "number" || typeof max !== "number") {
    throw new Error("The min and max parameters must be numbers");
  }
  if (max === 0) {
    throw new Error("The max parameter cannot be 0");
  }
  if (min > max) {
    throw new Error(
  ...
  if (min < 0 || max < 0) {
    throw new Error("The min and max parameters cannot be negative");
  }
  const result = [];
  for (let product of products) {
    if (!product.hasOwnProperty("priceInCents")) {
      throw new Error(
        "The products array contains an item without a priceInCents property"
      );
    }
