...
function filterProductsByPriceRange(products, min, max) {
  if (
    products.length === 0 || // - The `products` array is empty.
    typeof min !== "number" || // - Either `min` or `max` is not a number.
    typeof max !== "number" ||
    min > max || // - `min` is greater than `max`.
    min < 0 || // - Either `min` or `max` is less than `0`.
    max <= 0 // - `max` is equal to `0`.
  ) {
    throw 0;
  }
...
function getTotalOfAllProductsByPriceRange(products, min, max) {
  ...
  // getTotalOfAllProductsByPriceRange(products, "100", 50000);
